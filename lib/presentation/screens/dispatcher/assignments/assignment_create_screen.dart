...
class AssignmentCreateScreen extends ConsumerStatefulWidget {
  const AssignmentCreateScreen({Key? key}) : super(key: key);
...
class _AssignmentCreateScreenState
  ...
  Future<void> _loadData() async {
    ...
    // Obtener usuario actual (despachador)
    await userNotifier.fetchCurrentUser();
    _currentDispatcherId = ref.read(userProvider).currentUser?.id;
  ...
  Future<void> _submitForm() async {
    ...
    if (_currentDispatcherId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No se pudo obtener el despachador actual')),
      );
      return;
    }
    ...
    final success = await assignmentNotifier.createAssignment(request);
    ...
    if (success) {
      ...
      final errorMessage =
          ref.read(assignmentProvider).error ?? 'Error al crear asignación';
      ScaffoldMessenger.of(context)
